 ...
 */
 var rash_id = "#" + ($("md-tabs-content-wrapper md-tab-content div").last())[0].id + " ";
 var rash_id_suffix = "-" + rash_id.substr(1).trim();
 var footnote_regex = new RegExp(/#ft?n.*/);
...
jQuery.fn.extend({
    ...
    changeCSS: function(currentStyle) {
        if (currentStyle) {
            ...
            $(rash_id + "link[rel='stylesheet']").each(function() {
            ...
            if (currentStyle == "#rash_web_based_layout") { /* Transform to Web layout */
                $(rash_id + "link[rel='stylesheet']").remove();
                ...
                bootstrap_css.appendTo($(rash_id));
                rash_css.appendTo($(rash_id));
                $(rash_id + "#layoutselection").text("Web-based");
                $(this).hideCSS();
                $(this).addHeaderHTML();
    ...
    toggleCSS: function() {
        $(rash_id + ".footer ul").toggle();
    },
    hideCSS: function() {
        $(rash_id + ".footer ul").hide();
    },
    addHeaderHTML: function() {
        /* Reset header */
        //$(rash_id + "header").remove();
        //$("p.keywords").remove();
    
        /* Header title */
        var header = $("<header class=\"page-header container-fluid cgen\"></header>");
        header.prependTo($(rash_id + "section[role=doc-abstract]"))
        var title_string = "";
        var title_split = $(rash_id + "title").html().split(" -- ");
        ...
        /* Header author */
        var list_of_authors = [];
        $(rash_id + "meta[name='dc.creator']").each(function() {
            var current_value = $(this).attr("name");
            var current_id = $(this).attr("about");
            var current_name = $(this).attr("content");
            var current_email = $(rash_id + "meta[about='" + current_id + "'][property='schema:email']").attr("content");
            var current_affiliations = [];
            $(rash_id + "link[about='" + current_id + "'][property='schema:affiliation']").each(function() {
                var cur_affiliation_id = $(this).attr("href");
                current_affiliations.push($(rash_id + "meta[about='" + cur_affiliation_id + "'][property='schema:name']").attr("content"));
            });
        ...
        for (var i = 0; i < list_of_authors.length; i++) {
            ...
            if (i == 0) {
                author_element.insertAfter($(rash_id + "header h1"));
            } else {
                author_element.insertAfter($(rash_id + "header address:last-of-type"));
            }
        }
        /* /END Header author */
        
        /* ACM subjects */
        var categories = $(rash_id + "meta[name='dcterms.subject']");
        ...
        /* Keywords */
        var keywords = $(rash_id + "meta[property='prism:keyword']");
    ...
    addHeaderLNCS: function() {
        ...
        $(rash_id + "header .authors .affiliation").each(function () {
...
var figurebox_selector = rash_id + " figure > " + figurebox_selector_img + "," + rash_id + " figure > " + figurebox_selector_svg;
var tablebox_selector_table = "table";
var tablebox_selector = rash_id + " figure > " + tablebox_selector_table;
...
var formulabox_selector = 
    rash_id + " figure > " + formulabox_selector_img + "," + rash_id + " figure > " + formulabox_selector_span + "," + rash_id + " figure > " + formulabox_selector_math;
var listingbox_selector_pre = "pre";
var listingbox_selector = rash_id + " figure > " + listingbox_selector_pre;
...
$(function() {
    ...
    if (typeof MathJax !== 'undefined') {
        var s_delim = 's$s';
        var e_delim = 'e$e';
        $(rash_id).attr("class", "nomath");
        $(rash_id + 'span[role=math]').each(function() {
    ...
    /* Code Block */
    $(rash_id + 'pre').each(function() {
        $(this).html($.trim($(this).html()))
    });
    $(rash_id + 'pre > code').each(function() {
        $(this).html($.trim($(this).html()))
    });
    /* /END Code Block */

    /* Bibliographic reference list */
    $(rash_id + 'li[role=doc-biblioentry]').sort(function(a,b) {
    ...
    }).appendTo(rash_id + 'section[role=doc-bibliography] ul ,' + rash_id +'section[role=doc-bibliography] ol');
    
    /* Highlights in light-grey all the bibliographic references that are not cited in the paper */
    $(rash_id + 'li[role=doc-biblioentry]').each(function() {
    ...
    /* Append RASH VIEW suffix to footnotes sections */

    $(rash_id + "section[role=doc-footnote]").each(function(){
        var initial_ftn_id = $(this).attr("id");
        $(this).attr("id", initial_ftn_id + rash_id_suffix);
    });

    $(rash_id + "a[href]").each(function() {
        var is_ftn_ref = $(this).attr("href");
        if(footnote_regex.test(is_ftn_ref)){
            var actual_id = $(this).attr("href");
            $(this).attr("href", actual_id + rash_id_suffix);
        }
    });

    /* /END Append RASH VIEW suffix to footnotes sections */
    
    /* Footnotes (part one) */
    $(rash_id + 'section[role=doc-footnotes] section[role=doc-footnote]').sort(function(a,b) {
    ...
    }).appendTo(rash_id + 'section[role=doc-footnotes]');
    $(rash_id + "section[role=doc-footnotes]").prepend("<h1>Footnotes</h1>");
    ...
    /* Footnotes (part 2) */
    $(rash_id + "section[role=doc-footnotes] > section[role=doc-footnote]").each(function() {
    ...
    /* Container sections */
    $(
        rash_id + "> section ," + rash_id + "section[role=doc-abstract] ," + rash_id + "section[role=doc-acknowledgements] , " + rash_id + 
        "section[role=doc-bibliography]," + rash_id + "section[role=doc-footnotes]").addClass("container-fluid");
    /* /END Container sections */
    
    /* Heading dimensions */
    $(rash_id + "h1").each(function () {
        var counter = 0;
    ...
    /* Bibliography */
    $(rash_id + "section[role=doc-bibliography] ul").replaceWith("<ol>" +$(rash_id + "section[role=doc-bibliography] ul").html() + "</ol>");
    /* /END Bibliography */
